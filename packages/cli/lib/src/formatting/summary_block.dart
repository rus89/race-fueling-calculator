...
String formatSummaryBlock(FuelingPlan plan, {required bool useColor}) {
  ...
  final criticals = plan.warnings
      .where((w) => w.severity == Severity.critical)
      .toList();
  final advisories = plan.warnings
      .where((w) => w.severity == Severity.advisory)
      .toList();

  if (criticals.isNotEmpty || advisories.isNotEmpty) {
    buf.writeln('');
    buf.writeln(
      useColor ? bold('═══ WARNINGS ═══', enabled: true) : '=== WARNINGS ===',
    );
