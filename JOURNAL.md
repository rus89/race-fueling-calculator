...
## 2026-05-06 — Phase E (Diagnostics rail) complete

### What shipped
Three leaf widgets + one panel on `feat/v1.1-phase-e-diagnostics-rail`. 9 commits net (4 task implementations + 4 convergent review-fix commits + 1 E1-METRICS breadcrumb closeout). 242 app tests, 0 analyzer issues at branch close.

- **E1 — `RatioBar`** (`packages/app/lib/presentation/widgets/ratio_bar.dart`, 7 tests): glucose:fructose split bar with bullseye marker at 1.25 (sports-nutrition canonical 1:0.8) and shaded OK band 0.9–1.5 (same band PlanCanvas StatCard warns on). Doctrine-clean: `glu`/`fru` decorative bar fills; ideal marker in `ink`; OK band as full-zone tint at `ink @ 0.12 alpha` plus 1.5px edge borders at `0.45 alpha` (initial 0.18-alpha edge-only borders fell to ~1.20:1 contrast — WCAG 1.4.11 fail caught by a11y reviewer; bumped tint+stroke for ≥3:1 visibility). `assert(glucose.isFinite && fructose.isFinite && glucose >= 0 && fructose >= 0)` plus release-mode `glucoseShare.clamp(0.0, 1.0)`. Three-arm rendering: hasFructose / hasAnyCarb / neither (the both-zero branch renders `bg2` neutral-fill instead of misleading full-`glu`). Geometry pinned via `Key('ratio.idealMarker')` and `Key('ratio.okBand')` regression tests.
- **E2 — `CaffeineMeter`** (`packages/app/lib/presentation/widgets/caffeine_meter.dart`, 10 tests): 5-segment meter against the 6 mg/kg ceiling. `BonkTokens.caf` segments below ceiling, `BonkTokens.bad` at hot, redundant ` · OVER` text in `ink` per preflight note 4. Two real safety bugs surfaced and fixed: (a) `hot = filled >= _segs` fired at 5.4 mg/kg because `(0.9 × 5).round() = 5` — AT users were told "over ceiling" while at 90%; replaced with direct `hot = mgPerKg >= _ceilingMgPerKg`; (b) `bodyKg <= 0 ? 70 : bodyKg` let NaN through (`NaN <= 0` is `false`) → `NaN.clamp(0,5)` returned upper bound `5` → false full-red OVER alarm with `"Caffeine NaN mg/kg"` Semantics — replaced with `(bodyKg.isFinite && bodyKg > 0) ? bodyKg : 70.0`. Plus `mgEffective` coerce path for non-finite/negative `totalMg`. Visual hierarchy restored as plan's RichText (size-13 numeric + size-11 ink3 suffix) with tests opting into `findRichText: true`. Three contract pins at `totalMg=20` (sub-10% → 0 segments per plan note 8), `totalMg=378` (5.4 mg/kg → NOT hot, post-fix), `totalMg=420` (exact ceiling → hot).
- **E3 — `FlagCard`** (`packages/app/lib/presentation/widgets/flag_card.dart`, 10 tests): severity-tagged card. Color doctrine: `'CRITICAL'` / `'ADVISORY'` text in `ink` with the side rule (`Border(left: width: 3)`) carrying the severity color (`bad` / `warn`); body in `ink2`. `' — '` first-occurrence split via `indexOf + substring` (cleaner than plan's `split().skip().join()` and locks behavior under multi-em-dash inputs). Empty-message defensive: `title.isNotEmpty` and `body.isNotEmpty` guards collapse the row to just the badge + Semantics word. `maxLines: 2` on title and `maxLines: 6` on body with `overflow: ellipsis` to bound unbounded engine messages. `// TODO(v1.2)` breadcrumb on the heuristic — to be replaced with structured `Warning.title/detail` split. Side-rule width, body color, advisory Semantics word all pinned by tests.
- **E4 — `DiagnosticsRail`** (`packages/app/lib/presentation/panels/diagnostics_rail.dart`, 5 tests): right-pane panel branching on `planProvider` AsyncValue (PB-DATA-1) — loading wraps `CircularProgressIndicator` in `Semantics(liveRegion: true, label: 'Loading diagnostics')`; error renders `_ErrorFallback` (now also wrapped in `Semantics(liveRegion: true, container: true)` for parity with PlanCanvas — caught by both architecture and a11y reviewers); data composes the eyebrow, three section headers (each in `Semantics(header: true)` for AT navigation, Phase C parity), `RatioBar`, `CaffeineMeter`, and either `_AllClearCard` or per-warning `FlagCard`s. The `_AllClearCard` `BonkTokens.ok` dot measured 2.88:1 contrast on paper (WCAG 1.4.11 marginal-fail) — added a redundant `✓` glyph in `ink` to carry the green-light affordance non-color-dependently (StatCard pattern). NaN-finite guard on the `'CAFFEINE — N MG'` interpolation (`totalCaffeineMg.isFinite ? .round() : 0`) so a corrupt upstream value can't crash the section header even though `CaffeineMeter` itself coerces. `F1-RAIL-MIN-WIDTH:` breadcrumb at the class top documents that the panel needs ≥360px outer (320 inner after 20px horizontal padding) so RatioBar's 200% textScaler bound is honored — F1 must allocate ≥360 in the responsive layout.

### Engine notes
No engine changes in Phase E. `summary.totalGlucose` / `totalFructose` / `totalCaffeineMg` are Phase D additions; this phase only consumes them.

### Process notes — review pipeline impact
Each task ran a four-reviewer parallel pass (architecture / test coverage / a11y-UX / security with ethical-hacker mindset) followed by a convergent-fix subagent. The pipeline caught two real safety bugs (E2 `hot` threshold + NaN bypass; flagged independently by a11y and security reviewers) before they shipped, plus the WCAG 1.4.11 OK-band contrast (E1) and the `_AllClearCard` `ok` dot 2.88:1 (E4). Without the dual-axis review, both safety bugs would have shipped — `hot` because the plan's preflight note 8 only addressed the LOW end of the rounding contract, not the symmetric HIGH-end consequence; NaN because the original `<=` fallback looked complete but `NaN <= 0` is `false` and the assert that would have caught it strips in release.

### Plan-vs-reality drift caught (and fixed)

- Plan's verbatim E2 implementation rendered the readout via `RichText` but the verbatim tests used `find.textContaining(...)` without `findRichText: true` — `RichText` is opaque to that finder by default, so the plan's tests would have failed against the plan's implementation. Honored tests as contract; restored RichText for visual hierarchy and added the flag to the test calls.
- Plan's verbatim E2 Semantics label said `'milligrams per kilogram'` but the verbatim test asserted `contains('mg/kg')`. Same internal contradiction; tests-as-contract.
- Plan's verbatim test for the textScaler-200% case on a 320px-wide surface failed RatioBar's bottom Row layout (legend Row overflowed by ~230px at 2× scale). Replaced two `Row`s with `Wrap`s — preserves single-line layout at 1× and degrades gracefully at 2×.
- DiagnosticsRail at 320px outer leaves only 280px inner after 20+20 padding, below RatioBar's tested floor of 320px. Bumped the panel's textScaler test surface to 360px (= 320px inner) and breadcrumbed the production minimum width as `F1-RAIL-MIN-WIDTH:` — F1 must allocate ≥360px when wiring the three-pane responsive layout.

### Active carry-overs (still live for F1)

- **F1-RAIL-MIN-WIDTH** (`diagnostics_rail.dart`) — three-pane layout must allocate ≥360px to the diagnostics rail. New in Phase E.
- **F1-RESPONSIVE / F1-EMPTY-PLAN / F1-ERROR-COPY / F1-DOTS-SHAPE / PC-RESPONSIVE / PC-PRESERVE-DIST / PC-UNIT-CONVERSION / PB-SCROLL / PC-OUTLINED-BUTTON / PC-AID-VALIDATOR** — unchanged from Phase C/D.
- **PB-DATA-2 backlog** — unchanged.
- **`// TODO(v1.2):` structured `Warning.title/detail` split** (`flag_card.dart`) — replace the `' — '` heuristic when validators emit structured payloads. New in Phase E.

### Closed in Phase E

- **E1-METRICS** breadcrumb removed from `plan_canvas.dart` — DiagnosticsRail uses `summary.*` totals so no shared metric provider was needed (preflight note 9).
- **PB-A11Y-1 / PB-A11Y-4 / PB-A11Y-8** — already closed by Phase D; Phase E continued the doctrine.
