...
AllocationResult allocateProducts({
  ...
  // Check for depleted products
  for (final selection in selections) {
    final product = productMap[selection.productId];
    if (product != null && (remaining[selection.productId] ?? 0) <= 0) {
...
class _PlanCreateCommand extends Command<void> {
  ...
  _PlanCreateCommand(this.storage) {
    ...
    argParser.addOption('strategy', help: 'Strategy (steady/front-load/back-load/custom)', defaultsTo: 'steady');
  ...
  Strategy _parseStrategy(String s) {
    switch (s.toLowerCase().replaceAll('-', '').replaceAll('_', '')) {
      ...
      case 'custom': return Strategy.custom;
...
class _PlanProductsAddCommand extends Command<void> {
  ...
  _PlanProductsAddCommand(this.storage) {
    ...
    argParser.addFlag('aid-station', help: 'Product only available at aid stations');
  }
  ...
  Future<void> run() async {
    ...
    final isAidStation = results['aid-station'] as bool;
