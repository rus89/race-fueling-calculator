...
**v1.1 Phase E (Diagnostics rail): complete** on branch `feat/v1.1-phase-e-diagnostics-rail`. Three leaf widgets + one panel: `RatioBar` (glucose:fructose split bar with 1.25 bullseye + 0.9–1.5 OK band; full-zone ink-tint band + 1.5px borders for WCAG 1.4.11; finite/non-negative input asserts + share clamp; neutral 0/0 path; geometry pinned via `Key('ratio.idealMarker')` / `Key('ratio.okBand')`), `CaffeineMeter` (5-segment meter against 6 mg/kg ceiling with `hot` decoupled from rounded segment count via direct `mgPerKg >= ceiling` comparison; NaN-safe `bodyKg` fallback `(isFinite && >0) ? bodyKg : 70`; `mgEffective` coerce for non-finite/negative `totalMg`; redundant `· OVER` text in ink + RichText with `findRichText: true`), `FlagCard` (severity-tagged card with side-rule color carrying severity + `'CRITICAL'` / `'ADVISORY'` text in `ink`; `' — '` first-occurrence split via `indexOf+substring`; `maxLines: 2/6` + ellipsis bound; advisory Semantics word + body `ink2` + side-rule `width: 3` all pinned), `DiagnosticsRail` (right pane composing the three; AsyncValue branching with `_ErrorFallback` + `_AllClearCard` empty state with redundant `✓` glyph for the `ok` dot; `Semantics(header: true)` on section labels + `liveRegion` on `_ErrorFallback`; NaN guard on `totalCaffeineMg.round()` interpolation; `F1-RAIL-MIN-WIDTH` breadcrumb documenting ≥360px panel min-width). Closed `E1-METRICS` breadcrumb in `plan_canvas.dart`. DiagnosticsRail is **not yet wired into `PlannerPage`** — F1 does the three-pane layout. 242 app + 266 core + 279 cli tests.

Two real safety bugs caught and fixed by the parallel reviewer pipeline (a11y + security agents converged independently): (1) CaffeineMeter `hot = filled >= _segs` fired at 5.4 mg/kg (90% of ceiling) because `(0.9 × 5).round() = 5` — UI/AT announced "over ceiling" while user was 10% under — fixed by direct `mgPerKg >= 6.0` comparison; (2) `bodyKg <= 0 ? 70 : bodyKg` let `NaN` through (`NaN <= 0` is `false`) → `NaN/NaN*5` propagated, `.clamp(0,5)` returned upper bound `5`, false full-red OVER alarm with "Caffeine NaN mg/kg" Semantics — fixed by `(bodyKg.isFinite && bodyKg > 0)` guard. Both regressions locked by tests at `totalMg=378` (5.4 mg/kg, post-F1 NOT hot) and `totalMg=420` (exact ceiling, hot).

**v1.1 Phase F (Assembly + polish): complete** on branch `feat/v1.1-phase-f-assembly`. `PlannerPage` wired with `BonkTopbar` and a responsive three-pane body (`SetupRail` | `PlanCanvas` | `DiagnosticsRail`); `BonkBreakpoint.diagnosticsRailWidth` bumped to 380/360 to honor the ≥360 px floor (F1-RAIL-MIN-WIDTH closed) and wide threshold raised to 1520; mobile <880 collapses to tabs with the Diagnostics rail behind a Topbar "Checks" endDrawer (`usesEndDrawerForDiagnostics` getter gates the button). F1b shipped `BonkRecoveryBanner` with three branches (`PlanStorageException` → Retry + Discard, engine error → Retry, save failure → Retry save) plus `loadErrorProvider` and `PlannerNotifier.retrySave()`. F1c added stat-grid Wrap collapse <880 px, non-destructive empty-plan CTA, `resetToSeed()`, and `showSideRule: bool` parameter on rails so mobile tabs don't paint stray dividers. F1d closed PC-PRESERVE-DIST via sentinel-aware `RaceConfig.copyWith` for `distanceKm` / `intervalKm` only (other nullables keep null-as-no-change per documented rule); imperial unit conversion lives at the input boundary via `packages/app/lib/presentation/util/units.dart` (`kgToLb` / `lbToKg` / `kmToMi` / `miToKm`); `BonkFieldShell` labels embed the unit so AT users hear "Body mass (lb)". F2 debounced auto-save to a 500 ms quiescent window via `Debouncer<T>` (with `_disposed` flag); recovery flows pass `flushNow: true` through `_emitForce`. `SaveStatusController.beginSave({retrying = false})` implements sticky-failed (edits during failure don't clobber the signal; only explicit retry transitions through `InFlight`). Topbar `liveRegion: true` is unconditional so AT hears all transitions including success. F4 added an integration smoke test (`packages/app/integration_test/planner_flow_test.dart`) — compiles cleanly, web-only platform target. F3 visual goldens **deferred to v1.2** — `setUpGoogleFontsForTests` disables font fetching and stubs the asset manifest, which fundamentally conflicts with golden capture; retrofit cost is bundling Inter Tight + JetBrains Mono as real assets and updating the manifest stub. 326 app + 272 core + 279 cli = 877 tests total. `dart analyze` clean. F6 release tag pending Milan's merge.
